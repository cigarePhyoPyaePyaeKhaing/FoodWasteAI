...
const I18n = {
  ...
  formatUnitAggregate(items, getQuantity, getUnit, defaultUnit = '') {
    ...
    // If any unit has positive quantity, prune zero-quantity entries so they do not pollute mixed-unit display
    const hasAnyPositive = Object.values(totalsByUnit).some(v => v > 0);
    if (hasAnyPositive) {
      Object.keys(totalsByUnit).forEach(u => {
        if (totalsByUnit[u] <= 0) {
          delete totalsByUnit[u];
        }
      });
    }

    const activeUnits = Object.keys(totalsByUnit);
    ...
    return activeUnits.map(u => `${formatVal(totalsByUnit[u], u)} ${u}`).join(' • ');
  },
  ...
  translateDynamicText(text) {
    ...
    // 1. Operational Rules & Rationale
    if (trimmed.includes('assess_waste_risk') && trimmed.includes('Expired')) {
      return 'အန္တရာယ်မြင့်: သက်တမ်းကုန်ဆုံးသွားသဖြင့် ထုတ်လုပ်မှုရပ်ဆိုင်းပြီး စွန့်ပစ်ပါ သို့မဟုတ် မြေဆွေးပြုလုပ်ပါ';
    }
    ...
    if (trimmed.includes('evaluate_priority_use') && trimmed.includes('recommend_production')) {
      return 'ချက်ချင်း ဦးစားပေး သုံးစွဲပြီး ထုတ်လုပ်မှု ၂၀% လျှော့ချပါ';
    }
    if (trimmed.includes('evaluate_redistribution') && trimmed.includes('assess_waste_risk')) {
      return 'ပရဟိတ လှူဒါန်းရန် ပိုလျှံပစ္စည်းအဖြစ် အတည်ပြုသည်';
    }
    if (trimmed.includes('evaluate_redistribution')) {
      return 'ပရဟိတ လှူဒါန်းရန် သင့်တော်သော ပိုလျှံပစ္စည်းအဖြစ် အတည်ပြုသည်';
    }
    if (trimmed.includes('evaluate_priority_use') && (trimmed.includes('clear inventory within 3 days') || trimmed.includes('3 days'))) {
      return '၃ ရက်အတွင်း ကုန်စင်စေရန် ဦးစားပေးအဆင့်မြင့် သုံးစွဲပါ';
    }
    if (trimmed.includes('evaluate_priority_use')) {
      return 'သက်တမ်းကုန်ဆုံးရက် နီးကပ်နေသဖြင့် ချက်ချင်း ဦးစားပေး သုံးစွဲရန် လိုအပ်သည်';
    }
    if (trimmed.includes('recommend_production') && trimmed.includes('10-15%')) {
      return 'ထုတ်လုပ်မှု ၁၀-၁၅% အနည်းငယ် လျှော့ချပါ';
    }
    ...
    if ((lower.includes('surplus stock') || lower.includes('surplus') || lower.includes('projected surplus')) && (lower.includes('food bank') || lower.includes('partner') || lower.includes('dispatch'))) {
      const m = trimmed.match(/(?:surplus\s+stock|projected\s+surplus)\s*\(?([\d.]+)\s*([a-zA-Z]+)?\)?/i);
      if (m) {
        const qty = m[1];
        const unit = m[2] || 'kg';
        return `သက်တမ်းကုန်ခါနီး ပိုလျှံလက်ကျန် (${qty} ${unit}) တွေ့ရှိရပါသည်။ သက်တမ်းမကုန်မီ မှတ်ပုံတင်ထားသော အစားအစာဘဏ် သို့မဟုတ် မိတ်ဖက်အဖွဲ့အစည်းသို့ ပို့ဆောင်လှူဒါန်းပါ။`;
      }
      return 'သက်တမ်းမကုန်မီ ပိုလျှံနေသော အစားအစာများကို မှတ်ပုံတင်ထားသော မိတ်ဖက်အဖွဲ့အစည်း သို့မဟုတ် အစားအစာဘဏ်သို့ ပို့ဆောင်လှူဒါန်းပါ။';
    }
  ...
  translateStatus(status) {
    ...
    if (this.isMyanmar()) {
      ...
      if (upper === 'SAME_DAY_EXPIRY' || upper === 'SAMEDAYEXPIRY' || upper === 'EXPIRES_TODAY' || upper === 'EXPIRESTODAY') return 'ယနေ့ သက်တမ်းကုန်';
    ...
    if (upper === 'SAME_DAY_EXPIRY' || upper === 'SAMEDAYEXPIRY' || upper === 'EXPIRES_TODAY' || upper === 'EXPIRESTODAY') return 'Expires Today';
  ...
  translateWasteReason(reason) {
    ...
    if (this.isMyanmar()) {
      ...
      if (upper === 'EXPIRED') return 'သက်တမ်းကုန်';
      if (upper === 'UNSOLD') return 'မရောင်းရပိုလျှံ';
      if (upper === 'SPOILED') return 'ပျက်စီးဆုံးရှုံး';
      if (upper === 'DAMAGED') return 'ထိခိုက်ပျက်စီး';
      if (upper === 'PREPARATION_WASTE' || upper === 'PREPARATION') return 'ပြင်ဆင်မှုအလေအလွင့်';
      if (upper === 'OTHER') return 'အခြား';
    }
    if (upper === 'OVERPRODUCTION') return 'Overproduction';
    if (upper === 'EXPIRED') return 'Expired';
    if (upper === 'UNSOLD') return 'Unsold';
    if (upper === 'SPOILED') return 'Spoilage';
    if (upper === 'DAMAGED') return 'Damaged';
    if (upper === 'PREPARATION_WASTE' || upper === 'PREPARATION') return 'Preparation Waste';
    if (upper === 'OTHER') return 'Other';
    return reason;
  },

  translateWasteSource(notes) {
    if (!notes || notes === '-' || notes.trim() === '') return '-';
    const trimmed = notes.trim();
    const lower = trimmed.toLowerCase();
    
    if (this.isMyanmar()) {
      if (lower.includes('expired item disposal review') || lower.includes('disposal review')) {
        return 'သက်တမ်းကုန်ပစ္စည်း စွန့်ပစ်မှုစစ်ဆေးချက်မှ မှတ်တမ်းတင်ထားသည်';
      }
      if (lower.includes('already disposed') || lower.includes('already discarded')) {
        return 'စွန့်ပစ်ပြီးဖြစ်ကြောင်း အတည်ပြုမှတ်တမ်းတင်ထားသည်';
      }
      if (lower.includes('kitchen log entry') || lower.includes('kitchen log') || lower.includes('manual waste log')) {
        return 'မီးဖိုချောင်မှ မှတ်တမ်းတင်ထားသည်';
      }
      if (lower.includes('initial waste')) {
        return 'ကနဦး အလေအလွင့် မှတ်တမ်း';
      }
      if (lower.includes('dinner rush') || lower.includes('over-prepped')) {
        return 'ညစာစားချိန် ပိုလျှံပြင်ဆင်မိခြင်း';
      }
      if (/[\u1000-\u109F]/.test(trimmed)) {
        return trimmed;
      }
      return this.translateDynamicText(trimmed);
    } else {
      if (trimmed.includes('သက်တမ်းကုန်ပစ္စည်း') || trimmed.includes('စွန့်ပစ်မှုစစ်ဆေးချက်')) {
        return 'Recorded via Expired Item Disposal Review';
      }
      if (trimmed.includes('စွန့်ပစ်ပြီးဖြစ်ကြောင်း')) {
        return 'Confirmed as already disposed';
      }
      if (trimmed.includes('မီးဖိုချောင်')) {
        return 'Kitchen log entry';
      }
      return trimmed;
    }
  },
  ...
  translateFoodCategory(category) {
    ...
    if (this.isMyanmar()) {
      ...
      if (upper === 'POULTRY') return 'Poultry';
      if (upper === 'PRODUCE') return 'Produce';
      if (upper === 'SEAFOOD') return 'Seafood';
      if (upper === 'GRAINS') return 'Grains';
      if (upper === 'DAIRY') return 'Dairy';
      if (upper === 'BAKERY') return 'Bakery';
      if (upper === 'MEAT') return 'Meat';
      if (upper === 'OTHER') return 'Other';
    }
    return category;
  },
  ...
  translateDay(day) {
    ...
    if (this.isMyanmar()) {
      ...
      if (str === 'Tuesday' || str.startsWith('Tue')) return (str.includes('Today') || str.includes('ယနေ့')) ? 'အင်္ဂါ (ယနေ့)' : 'အင်္ဂါ';
  ...
  renderLanguageSwitcher() {
    ...
    // 1. Settings Page Dedicated Full Selector
    const settingsContainer = document.getElementById('settings-language-container');
    if (settingsContainer) {
      settingsContainer.innerHTML = `
        <div class="lang-switch-bubble" role="group" aria-label="Language selection">
          <button type="button" id="settings-lang-btn-en" data-lang-val="en" onclick="I18n.setLanguage('en')" class="lang-btn ${isEn ? 'active' : ''}">
            🌐 English
          </button>
          <button type="button" id="settings-lang-btn-mm" data-lang-val="mm" onclick="I18n.setLanguage('mm')" class="lang-btn ${isMm ? 'active' : ''}">
            မြန်မာ
          </button>
        </div>
      `;
    ...
    // 2. Global Header / Shell Container (Compact Desktop Language Selector)
  ...
  updateSwitcherUI() {
    ...
    document.querySelectorAll('#lang-btn-en, #settings-lang-btn-en').forEach(btn => {
      if (isEn) {
    ...
    document.querySelectorAll('#lang-btn-mm, #settings-lang-btn-mm').forEach(btn => {
      if (isMm) {
        btn.classList.add('active');
        btn.setAttribute('aria-pressed', 'true');
      } else {
    ...
    if (langLabel) {
      langLabel.textContent = isMm ? 'မြန်မာ' : 'English';
    }
  }
};
