...
public class ExpiryStatusResolver {
    ...
    /**
     * Resolves the database status column string ('EXPIRED', 'NEAR_EXPIRY', 'OK').
     * Compatible with MySQL ENUM('OK', 'NEAR_EXPIRY', 'EXPIRED').
     */
    public static String resolveStatus(LocalDate expiryDate, LocalDate today) {
        LocalDate current = (today != null) ? today : getToday();
    ...
    public static boolean isNearOrSameDayExpiry(LocalDate expiryDate, LocalDate today) {
        ExpiryState s = resolveState(expiryDate, today);
        return s == ExpiryState.SAME_DAY_EXPIRY || s == ExpiryState.NEAR_EXPIRY;
    }

    public static boolean isExpiresToday(LocalDate expiryDate, LocalDate today) {
        if (expiryDate == null) return false;
        LocalDate current = (today != null) ? today : getToday();
        return expiryDate.isEqual(current);
    }

    public static boolean isTomorrowCandidate(LocalDate expiryDate, LocalDate today) {
        if (expiryDate == null) return false;
        LocalDate current = (today != null) ? today : getToday();
        return expiryDate.isEqual(current.plusDays(1));
    }
