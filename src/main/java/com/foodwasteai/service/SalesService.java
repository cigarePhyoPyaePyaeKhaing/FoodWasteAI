...
public class SalesService {
    ...
    private static final Map<String, Sale> processedClientRequests = new ConcurrentHashMap<>();
    ...
    public Sale recordSale(Sale sale, Long userId) throws SQLException {
        ...
        if (sale.getClientRequestId() != null && !sale.getClientRequestId().trim().isEmpty()) {
            ...
            synchronized (lock) {
                try {
                    ...
                    if (existing != null) {
                        logger.warn("Idempotent duplicate sale request blocked in critical section (token: '{}'). Returning existing sale record #{}.", token, existing.getId());
                        return existing;
                    }

                    Sale saved;
                    if (DatabaseConfig.isAvailable()) {
                        saved = salesDao.recordSaleWithStockDeduction(sale, userId);
                    } else {
                        saved = recordSaleInMemory(sale, userId);
                    }
        ...
        if (DatabaseConfig.isAvailable()) {
            ...
            return recordSaleInMemory(sale, userId);
        }
    }
