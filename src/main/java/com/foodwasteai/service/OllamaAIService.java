...
/**
 * Free local AI conversational assistant powered by Ollama (qwen2.5:3b) and SWI-Prolog:
 * User -> ChatServlet -> Java Backend -> MySQL Data -> SWI-Prolog Reasoning -> Ollama / Grounded XAI -> Smart Directives
...
public class OllamaAIService {
    ...
    /**
     * Executes the complete free AI pipeline:
     * User Query -> Intent Analysis -> Live MySQL Data -> SWI-Prolog Reasoning -> Ollama (qwen2.5:3b) / Rule-Grounded Explanation
     */
    ...
    public ChatResponse processUserQuery(String userQuery, String language) {
        ...
        try {
            ...
            // 5. Synthesize Explanation (Ollama local model or Intelligent Grounded Fallback)
            String ollamaExplanation = callOllamaApi(userQuery, inventory, items, recipients, activeLang);

            if (ollamaExplanation == null || ollamaExplanation.trim().isEmpty()) {
                ollamaExplanation = generateRuleGroundedExplanation(userQuery, inventory, items, recipients, matchedFoodItem, matchedAssessment, activeLang);
                ...
                response.setSourceEngine("Ollama (" + AppConfig.getOllamaModel() + ") + SWI-Prolog Ground Truth");
                response.addSource("Ollama Local AI (" + AppConfig.getOllamaModel() + ")");
            }

            response.setAnswer(ollamaExplanation);
            ...
            logger.error("Error in OllamaAIService: {}", e.getMessage(), e);
            if (isMyanmar) {
    ...
    /**
     * Sends prompt to local Ollama generate API (http://localhost:11434/api/generate)
     */
    private String callOllamaApi(String userQuery, List<FoodItem> inventory, List<PrologAssessment> prologAssessments,
                                 List<RedistributionRecipient> recipients, String lang) {
        try {
            String host = AppConfig.getOllamaHost();
            String model = AppConfig.getOllamaModel();
            String endpoint = host.replaceAll("/+$", "") + "/api/generate";

            StringBuilder promptBuilder = new StringBuilder();
            promptBuilder.append("You are the Explainable AI Assistant for FoodWaste AI, an intelligent conversational system for food waste prediction, prevention, and redistribution.\n\n");
            promptBuilder.append("CURRENT INVENTORY & DATABASE METRICS (MySQL):\n");
            for (FoodItem item : inventory) {
                promptBuilder.append(String.format("- %s (ID %d): Stock=%.1f %s, Price=%s MMK/unit, Expiry=%s, Status=%s, ExpiryStatus=%s\n",
            ...
            for (PrologAssessment a : prologAssessments) {
                promptBuilder.append(String.format("- %s: Risk=%s (%d%%), Priority=%s, Reasons=%s, RecAction=%s, Redistribute=%s\n",
            ...
            for (RedistributionRecipient r : recipients) {
                promptBuilder.append(String.format("- %s (%s, Contact: %s, Phone: %s)\n", r.getName(), r.getOrganizationType(), r.getContactPerson(), r.getPhone()));
            }

            promptBuilder.append("\nINSTRUCTIONS FOR AI:\n");
            if ("mm".equalsIgnoreCase(lang)) {
                promptBuilder.append("1. Answer the user's question in natural, highly articulate Professional Business Myanmar (Burmese) language using Myanmar Unicode script.\n");
            } else {
                promptBuilder.append("1. Answer the user's question concisely, clearly, and politely using an articulate, professional English tone.\n");
            }
            promptBuilder.append("2. Strictly adhere to the SWI-Prolog logical conclusions and MySQL metrics above. Do not invent contradictory numbers.\n");
            promptBuilder.append("3. Always preserve exact food names, numbers, units (liter, kg, MMK, pieces), and Prolog predicates (assess_waste_risk/6, evaluate_priority_use/3, recommend_production/6, evaluate_redistribution/6).\n");
            promptBuilder.append("4. Format with clean markdown: bold headings, bullet points, and clear actionable mitigation directives.\n\n");
            promptBuilder.append("USER QUESTION: ").append(userQuery);
            ...
            if (httpResponse.statusCode() == 200) {
                ...
                logger.warn("Ollama API call returned status {}: {}", httpResponse.statusCode(), httpResponse.body());
            }
        } catch (Exception e) {
            logger.debug("Ollama local AI is offline or unreachable ({}). Using grounded XAI synthesizer fallback.", e.getMessage());
        }
        return null;
    }
