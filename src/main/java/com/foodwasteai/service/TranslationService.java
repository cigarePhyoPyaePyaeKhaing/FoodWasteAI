...
public class TranslationService {
    ...
    private String synthesizeMyanmarText(String text) {
        ...
        // e.g. "Projected surplus (7.4 liter) detected near expiry. Dispatch to registered food bank or partner organization before expiry cutoff."
        if ((lower.contains("surplus stock") || lower.contains("surplus") || lower.contains("projected surplus")) && (lower.contains("food bank") || lower.contains("partner") || lower.contains("dispatch"))) {
            Matcher m = Pattern.compile("(?:surplus\\s+stock|projected\\s+surplus)\\s*\\(?([\\d.]+)\\s*([a-zA-Z]+)?\\)?", Pattern.CASE_INSENSITIVE).matcher(trimmed);
            if (m.find()) {
                String qty = m.group(1);
                String unit = m.group(2) != null ? m.group(2) : "kg";
                return String.format("သက်တမ်းကုန်ခါနီး ပိုလျှံလက်ကျန် (%s %s) တွေ့ရှိရပါသည်။ သက်တမ်းမကုန်မီ မှတ်ပုံတင်ထားသော အစားအစာဘဏ် သို့မဟုတ် မိတ်ဖက်အဖွဲ့အစည်းသို့ ပို့ဆောင်လှူဒါန်းပါ။", qty, unit);
            }
